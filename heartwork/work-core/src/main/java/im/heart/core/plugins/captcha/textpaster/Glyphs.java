...
public class Glyphs {
	  ...
	  // List<GlyphAbsolutePositionAndRenderContext> vectors = new ArrayList<GlyphAbsolutePositionAndRenderContext>();
	    ...
	    public void addGlyphVector(GlyphVector glyph){
	        this.vectors.add(glyph);
	                ...
	                //new GlyphAbsolutePositionAndRenderContext(glyph,frc,new Point2D.Float(0,0)));
	    }
	   ...
	   public void addAffineTransform(int index,AffineTransform at){
	       ...
	       if(t==null){t=at;}else{t.concatenate(at);}
	       vectors.get(index).setGlyphTransform(0,t);
	   }

	   public void rotate(int index, double angle){
	       get(index).setGlyphTransform(0, AffineTransform.getRotateInstance(angle,getBoundsX(index)+getBoundsWidth(index)/2,getBoundsY(index)+getBoundsHeight(index)/2));
	   }
