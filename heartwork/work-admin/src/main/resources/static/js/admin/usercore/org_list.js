define(function(require, exports, moudles) {
	...
	var relateOrgView=function(id,dataId){
		$.httpUtil.curl({
			url : url.api  +"/"+ dataId+ "/relate.jhtml",
			loading : false,
			success : function(data) {
				dialog({
					id : id,
	...
	var addChild=function(id,dataId){
		$.httpUtil.curl({
	...
	$tbody.on("click", ".operate .btn-add-child",function() {
		var dataId = $(this).attr("data");
		var id = $(this).attr("id");
		addChild(id,dataId);
	 });
	var deleteOrg=function(id,dataId){
		var d = dialog({
			id : id,
			title : '消息',
			content : '是否要删除？',
